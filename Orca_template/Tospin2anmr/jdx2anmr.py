...
outfile_name = f"{args.out}.dat"

lines: list[str] = open(args.file, "r").readlines()
idx0_lines_start: int
idx0_lines_end: int
...
for idx, line in enumerate(lines):
    if re.search(r"END", line):
        idx0_lines_end = idx
if (not idx0_lines_start) or (not idx0_lines_end):
    print("  Your jdx format file have something wrong !!!")
    print("  Close and exit the program !!!")
    exit(0)

idx0_lines_start = idx0_lines_start+1
idx0_lines_end = idx0_lines_end-1
outData: list = []
TotalNums: int = 0
