...
class PauliComposer:
    ...
    def __init__(self, entry: str, weight: Number | None = None) -> None:
        ...
        if weight is not None:
            if self.iscomplex:
                ...
                ent = np.full(self.dim, float(self.init_entry))  # type: ignore
        else:
            if self.iscomplex:
    ...
    def to_sparse(self) -> ss.csr_matrix:
