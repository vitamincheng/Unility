...
def _process_QM_hydrogen_spin_system(inParameter: tuple[npt.NDArray[np.float64], npt.NDArray[np.float64],  list[int]], ab_group_sets: list[set[int]], mat_filter_multi: npt.NDArray[np.uint8], inAnmr: Anmr, args: argparse.Namespace) -> tuple[list[int], list[list[tuple[float, float]]]]:
    ...
    if np.sum(inSParams.astype(bool)*inHydrogen) <= args.mss:
        # use all atoms in ab_group_sets (the same of every row)
        ab_group: list[int] = list(ab_group_sets[0])
        freq: npt.NDArray[np.float64] = inSParams[ab_group]
        JCoups: npt.NDArray[np.float64] = inJCoups[ab_group].T[ab_group]
        ...
        QM_Bases: list[tuple[float, float]] = qm_full(
            freq=list(freq), JCoups=JCoups, _verbose=args.verbose, _cutoff=args.cutoff)
        ...
        for idx0, ab_group_set in enumerate(ab_group_sets):
            ...
            # if in any element of ab_group_sets have more than one Hydrogen,
            ...
            freq: npt.NDArray[np.float64] = inSParams[idx0_ab_group]
            JCoups: npt.NDArray[np.float64] = inJCoups[idx0_ab_group].T[idx0_ab_group]

            QM_Bases: list[tuple[float, float]] = qm_base(freq=list(
                freq), JCoups=JCoups, idx0_nspins=idx0_ab_group.index(idx0), _cutoff=args.cutoff, _verbose=args.verbose)

            QM_Multiplet: list[tuple[float, float]] = []
            # for QM_base in QM_Bases:
            for bases_freq, bases_intensit in QM_Bases:
                ...
                if len(inJCoups_multi) >= 1:
                    tmp: npt.NDArray[np.float64] = np.array(
                        qm_multiplet(bases_freq, inHydrogen[idx0], JCoups=inJCoups_multi, delta=delta_SParams, _verbose=args.verbose))
                    tmp.T[1] *= bases_intensit
