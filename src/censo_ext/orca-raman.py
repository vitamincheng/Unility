...
# global constants
found_raman_section = False  # check for Raman data in out
specstring_start = 'RAMAN SPECTRUM'  # check orca.out from here
...
# go from high to low wave number, normal for IR spectra, low wn to high wn if False
high_to_low_wn = True
transm_style = True  # show spectra in transmittance style, absorption style if False
show_grid = True  # show grid if True
# show the convoluted spectra if True (if False peak labels will not be shown)
...
linear_locator = True
spectrum_title = "RAMAN spectrum"  # title
# weight of the title font: 'normal' | 'bold' | 'heavy' | 'light' | 'ultrabold' | 'ultralight'
...
parser = argparse.ArgumentParser(
    prog='orca_raman', description='Easily plot RAMAN spectra from orca.out')
    ...
    with open(args.filename, "r") as input_file:
        for line in input_file:
            ...
            if line.startswith(specstring_start):
                # found Raman data in orca.out
                found_raman_section = True
                for line in input_file:
                    # stop exctract text
                    ...
                    if re.search("\d:", line):
                        ...
                        intenslist.append(
                            float(line.strip().split()[intens_column]))
# file not found -> exit here
except IOError:
    print(f"'{args.filename}'" + " not found")
    sys.exit(1)

# no Raman data in orca.out -> exit here
if not found_raman_section:
...
for index, freq in enumerate(freqlist):
    ...
    if show_single_gauss:
        ax.plot(plt_range_x, gauss(
            intenslist[index], plt_range_x, freq, w), color="grey", alpha=0.5)
    # single gauss function filled plot
    if show_single_gauss_area:
        ax.fill_between(plt_range_x, gauss(
            intenslist[index], plt_range_x, freq, w), color="grey", alpha=0.5)
...
if transm_style:
    plt.ylim(max(plt_range_gauss_sum_y) +
             max(plt_range_gauss_sum_y)*0.1, 0)  # +10% for labels
else:
    plt.ylim(0, max(plt_range_gauss_sum_y) +
             max(plt_range_gauss_sum_y)*0.1)  # +10% for labels
...
if save_spectrum:
    ...
    plt.savefig(f"{filename}-raman" + ".pdf", dpi=figure_dpi)
